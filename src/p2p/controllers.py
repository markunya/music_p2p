...
class AttentionControl(abc.ABC):
    ...
    def __init__(self, num_diffusion_steps: int):
        self._diffusion_step = (
            0  # must be set externally at each step through set_diffusion_step
        )
        ...
        if len(self.attention_store) == 0:
            ...
            for i in range(len(self.attention_store)):
                self.attention_store[i] = (
                    1 - self.running_mean_coef
                ) * self.attention_store[i] + self.running_mean_coef * self.step_store[
                    i
                ]
...
class AttentionControlReplace(AttentionControlEdit, abc.ABC):
    def __init__(
        ...
        self.diffusion_step_start = (
            0 if diffusion_step_start is None else diffusion_step_start
        )
        self.diffusion_step_end = (
            num_diffusion_steps if diffusion_step_end is None else diffusion_step_end
        )

    def _get_alpha_from_cosine_schedule(self):
        step = min(
            max(self._diffusion_step, self.diffusion_step_start),
            self.diffusion_step_end,
        )
        ...
        scale = 0.5 * (
            1 + math.cos(math.pi * (step - self.diffusion_step_start) / denom)
        )
        return self.eta_min + (self.eta_max - self.eta_min) * scale

    def _replace_cross_attention_impl(
        self, attn_weight_base, attn_weight_replace, mapper_pos_src, mapper_pos_tgt
    ):
        len_src_seq = attn_weight_base.shape[-1]
        len_tgt_seq = attn_weight_replace.shape[-1]

        M_full = attn_weight_replace.new_zeros(
            (self.batch_size - 1, len_src_seq, len_tgt_seq)
        )
        ...
        M_full[
            ...
            mapper_pos_tgt : mapper_pos_tgt + self.mapper.shape[-1],
        ] = self.mapper

        edit_mask_full = torch.zeros(
            M_full.shape[0], M_full.shape[-1], device=M_full.device
        )
        edit_mask_full[
            :, mapper_pos_tgt : mapper_pos_tgt + self.edit_mask.shape[-1]
        ] = self.edit_mask

        attn_weight_base_mapped = torch.einsum(
            "hpw,bwn->bhpn", attn_weight_base, M_full
        )
        alpha = self._get_alpha_from_cosine_schedule()

        interpolation = (
            alpha * attn_weight_replace + (1 - alpha) * attn_weight_base_mapped
        )
        return interpolation * edit_mask_full + attn_weight_base_mapped * (
            1 - edit_mask_full
        )
...
class AttentionReplaceLyrics(AttentionControlReplace):
    ...
    def replace_cross_attention(self, attn_weight_base, attn_weight_replace):
        ...
        return self._replace_cross_attention_impl(
            attn_weight_base, attn_weight_replace, mapper_pos_src, mapper_pos_tgt
        )
...
class AttentionReplaceTags(AttentionControlReplace):
    ...
    def replace_cross_attention(self, attn_weight_base, attn_weight_replace):
        return self._replace_cross_attention_impl(
            attn_weight_base, attn_weight_replace, 1, 1
        )
